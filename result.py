...
df1 = pd.pivot_table(df, columns=["factor", "year"], values=["res"], aggfunc=np.sum, fill_value=np.nan).reset_index(
    drop=True)  # .rename(columns={'res':'1'})
world = {0: df1.sum(axis=1)}
df1 = df1.rename(index=world)
df1

df2 = pd.pivot_table(df, columns=["factor", "year"], values=["res"], aggfunc=np.sum, fill_value=np.nan).reset_index(
    drop=True)  # .rename(columns={'res':'1'})
df2 = df2[1].join(df2[2], rsuffix='_r', lsuffix='_l').join(df2[2] / df2[1], rsuffix='_r', lsuffix='_l').rename(
    index=world)
df2.columns = pd.MultiIndex.from_tuples(zip([1] * 13 + [2] * 13 + [6] * 13, pd.unique(df['year']).tolist() * 3),
                                        names=['factor', 'year'])
df2
